 ...
 */
void copy_board(const stone_t board[HIGHT][AREA],
		stone_t dest[HIGHT][AREA]) {
 ...
 */
void copy_board_diagonally(int pos, const stone_t board[HIGHT][AREA],
			   stone_t dest[AREA]) {
  int i, j;
  if (pos % 5 == 0) {
    // 0, 5, 10, 15をコピー
 ...
 * @param stone_t dest[] 結果の面
 */
void copy_board_latitudinally(int pos,
			      const stone_t board[HIGHT][AREA],
			      stone_t dest[AREA]) {
  int i, j;
  for (i = 0; i < HIGHT; i++) {
    for (j = 0; j < WIDTH; j++) {
 ...
 * @param stone_t dest[] 結果の面
 */
void copy_board_longitudinally(int pos,
			       const stone_t board[HIGHT][AREA],
			       stone_t dest[AREA]) {
  int i, j;
  for (i = 0; i < HIGHT; i++) {
 ...
 */
int check_win_plane(int pos, stone_t color,
		    const stone_t plane[AREA]) {
  int win;
  int i;
  // 斜めの処理
  if (pos % 5 == 0) {
 ...
 * @return int true:1, false:0
 */
int check_win(int layer, int pos, stone_t color,
	      const stone_t board[HIGHT][AREA]) {
  stone_t temp[AREA];
 ...
 * @return int true:1, false:0
 */
int is_terminate_hand(int pos, stone_t turn,
		      const stone_t board[HIGHT][AREA]) {
