 ...
 */
int playout(int first, stone_t mycolor, stone_t turn,
 ...
 */
void expand_child(node_t mother, stone_t mycolor, stone_t turn,
  ...
  for (i = 0; i < mother->nchildren; i++) {
    ...
    total_win +=
      mother->children[i]->win = playout(hands[i], mycolor, turn, copy);
  }
  // 上位にも反映
...
node_t select_child(node_t mother, int sign) {
  ...
  for (i = 0; i < mother->nchildren; i++) {
    ...
    temp_ucb = ucb(sign * winning_rate, get_total_playout(mother),
		   mother->children[i]->playout);
    if (max_ucb < temp_ucb) {
      max_ucb = temp_ucb;
      max_ucb_index = i;
    }
 ...
 */
node_t search_candidate(int sign, node_t mother, stone_t mycolor,
			stone_t* turn, stone_t board[HIGHT][AREA]) {
  // 最善手を選ぶ
